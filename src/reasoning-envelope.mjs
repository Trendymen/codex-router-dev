...
function payloadShape(payload) {
  ...
  if (payload.signature !== null && (typeof payload.signature !== "string" || !payload.signature)) throw new TypeError("reasoning envelope payload has invalid signature");
  return payload;
}
...
function expectedMatches(payload, expected) {
  if (!expected || typeof expected !== "object") return { route: false, hash: false };
  const route = ["provider", "model", "transport", "responseId", "itemId"].every((field) => expected[field] === undefined || payload[field] === expected[field]);
  const hash = expected.textSha256 === undefined && expected.summaryParts === undefined
    ? true
    : expected.textSha256 !== undefined ? payload.textSha256 === expected.textSha256 : payload.textSha256 === reasoningTextHash(expected.summaryParts);
  return { route, hash };
}
...
export function verifyReasoningEnvelope(value, expected, internalKey) {
  ...
  let bytes;
  try { bytes = Buffer.from(encoded[0], "base64url"); if (bytes.toString("base64url") !== encoded[0]) return verdict("invalid", { code: "thinking_signature_invalid" }); const payload = JSON.parse(bytes.toString("utf8")); payloadShape(payload); const provided = Buffer.from(encoded[1], "base64url"); if (provided.toString("base64url") !== encoded[1] || provided.length !== 32) return verdict("invalid", { code: "thinking_signature_invalid" });
    ...
    if (!match.hash) return verdict("invalid", { code: "thinking_signature_invalid", payload });
    if (!match.route) return verdict("foreign", { code: "thinking_provenance_foreign", payload });
