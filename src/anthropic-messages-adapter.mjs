...
class AnthropicStreamTransform extends Transform {
  ...
  #processBuffered() { let boundary; while (!this.#backpressured && (boundary = frameBoundary(this.#buffer))) { const frame = this.#buffer.subarray(0, boundary.index); this.#buffer = this.#buffer.subarray(boundary.index + boundary.length); this.#consume(frame); } if (this.#buffer.length > MAX_FRAME_BYTES) fail("provider_response_malformed"); }
  ...
  #emit(type, value) { if (type !== "[DONE]" && value === undefined) return; let output = value; if (type !== "[DONE]" && this.#toolBuild?.mapping && output && typeof output === "object") output = restoreToolEvent(output, this.#toolBuild.mapping); if (type !== "[DONE]" && output === undefined) return; if (type !== "[DONE]" && output && typeof output === "object") output = { ...output, sequence_number: ++this.#state.sequence }; const chunk = Buffer.from(type === "[DONE]" ? "data: [DONE]\n\n" : eventFrame(type, output), "utf8"); const limit = type === "[DONE]" || type === "response.completed" || type === "response.incomplete" || type === "response.failed" ? MAX_BODY_BYTES : MAX_BODY_BYTES - TERMINAL_QUEUE_RESERVE; if (this.#queueBytes + chunk.length > limit) fail("provider_response_malformed"); this.#queueBytes += chunk.length; this.#queue.push(chunk); this.#drain(); }
  #spend() { this.#state.work += 1; if (this.#state.work > MAX_WORK) fail("provider_response_malformed"); }
  #failSafe(code) { if (this.#state.terminal) { try { this.#requestContext.observeReasoningProtocol?.({ code: "event_after_terminal" }); } catch {} return; } this.#callAbort(); try { this.#terminal("failed", code); } catch { this.#state.terminal = true; this.#emit("response.failed", { type: "response.failed", response: { id: this.#state.id || this.#requestContext.responseId || "resp_unknown", object: "response", status: "failed", model: this.#state.model || this.#model.slug || this.#model.upstreamModel, created_at: this.#state.created ?? 0, output: [...this.#state.items.values()].sort((a, b) => a.index - b.index).map((item) => item.output), usage: this.#state.usage ?? null, error: { type: "router_error", code, message: code, param: null, details: null } } }); this.#emit("[DONE]"); } }
