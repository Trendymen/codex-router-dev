...
export function buildAnthropicMessagesRequest({ model, payload, credential, internalKey, requestContext = {} } = {}) {
  ...
  const toolBuild = encodeToolDialect({ tools: source.tools, toolChoice: source.tool_choice, input: normalizedInput, profile: model.requestProfile ?? model.provider });
  ...
  if (choice && tools?.length) json.tool_choice = choice;
...
function frameBoundary(buffer) { for (let index = 0; index < buffer.length; index += 1) { const first = lineLength(buffer, index); if (!first) continue; const second = lineLength(buffer, index + first); if (second) return { index, length: first + second }; } return undefined; }
...
class AnthropicStreamTransform extends Transform {
  ...
  _transform(chunk, _encoding, callback) { try { const bytes = Buffer.from(chunk); this.#state.bodyBytes += bytes.length; if (this.#state.bodyBytes > MAX_BODY_BYTES) fail("provider_response_malformed"); this.#buffer = Buffer.concat([this.#buffer, bytes]); if (this.#buffer.length > MAX_FRAME_BYTES) fail("provider_response_malformed"); let boundary; while ((boundary = frameBoundary(this.#buffer))) { const frame = this.#buffer.subarray(0, boundary.index); this.#buffer = this.#buffer.subarray(boundary.index + boundary.length); this.#consume(frame); } if (this.#buffer.length > MAX_FRAME_BYTES) fail("provider_response_malformed"); this.#drain(); if (this.#backpressured) this.#pendingTransform = callback; else callback(); } catch (error) { if (error?.code === "request_aborted") callback(error); else { this.#failSafe(error?.code || "provider_response_malformed"); this.#drain(); if (this.#backpressured) this.#pendingTransform = callback; else callback(); } } }
  _flush(callback) { try { if (this.#buffer.length) { const parsed = parseFrame(this.#buffer); this.#buffer = Buffer.alloc(0); if (parsed) this.#consumeParsed(parsed); } if (!this.#state.terminal) { if (!this.#state.messageStopped) fail("upstream_stream_truncated"); this.#terminal(this.#state.stopReason === "max_tokens" ? "incomplete" : "completed"); } this.#drain(); if (this.#backpressured || this.#queue.length) this.#pendingFlush = callback; else callback(); } catch (error) { if (error?.code === "request_aborted") callback(error); else { this.#failSafe(error?.code || "upstream_stream_truncated"); this.#drain(); if (this.#backpressured || this.#queue.length) this.#pendingFlush = callback; else callback(); } } }
  _read() { this.#backpressured = false; this.#drain(); }
  #drain() { while (!this.#backpressured && this.#queue.length) { const chunk = this.#queue.shift(); this.#queueBytes -= chunk.length; if (!this.push(chunk)) this.#backpressured = true; } if (!this.#backpressured && !this.#queue.length) { const transform = this.#pendingTransform; this.#pendingTransform = undefined; transform?.(); const flush = this.#pendingFlush; this.#pendingFlush = undefined; flush?.(); } }
  #emit(type, value) { if (type !== "[DONE]" && value === undefined) return; let output = value; if (type !== "[DONE]" && this.#toolBuild?.mapping && output && typeof output === "object") output = restoreToolEvent(output, this.#toolBuild.mapping); if (type !== "[DONE]" && output === undefined) return; if (type !== "[DONE]" && output && typeof output === "object") output = { ...output, sequence_number: ++this.#state.sequence }; const chunk = Buffer.from(type === "[DONE]" ? "data: [DONE]\n\n" : eventFrame(type, output), "utf8"); this.#queueBytes += chunk.length; if (this.#queueBytes > MAX_BODY_BYTES) fail("provider_response_malformed"); this.#queue.push(chunk); this.#drain(); }
  #spend() { this.#state.work += 1; if (this.#state.work > MAX_WORK) fail("provider_response_malformed"); }
  #failSafe(code) { if (this.#state.terminal) { try { this.#requestContext.observeReasoningProtocol?.({ code: "event_after_terminal" }); } catch {} return; } this.#callAbort(); try { this.#terminal("failed", code); } catch { this.#state.terminal = true; this.#emit("response.failed", { type: "response.failed", response: { id: this.#state.id || "resp_unknown", object: "response", status: "failed", output: [], error: { code } } }); this.#emit("[DONE]"); } }
  #consume(frame) { const parsed = parseFrame(frame); if (parsed) this.#consumeParsed(parsed); }
  #consumeParsed(parsed) { if (parsed.done) { if (this.#state.terminal) try { this.#requestContext.observeReasoningProtocol?.({ code: "event_after_terminal" }); } catch {} return; } const value = parsed.value; if (!value || typeof value !== "object") fail("provider_response_malformed"); if (this.#state.terminal) { try { this.#requestContext.observeReasoningProtocol?.({ code: "event_after_terminal" }); } catch {} return; } this.#spend(); if (value.type === "message_start") return this.#messageStart(value); if (!this.#state.messageStarted) fail("provider_response_malformed"); if (value.type === "content_block_start") return this.#start(value); if (value.type === "content_block_delta") return this.#delta(value); if (value.type === "content_block_stop") return this.#stopBlock(value); if (value.type === "message_delta") { const delta = object(value.delta); if (delta.stop_reason !== undefined && !["end_turn", "tool_use", "max_tokens"].includes(delta.stop_reason)) fail("provider_response_malformed"); if (delta.stop_reason !== undefined) this.#state.stopReason = delta.stop_reason; if (value.usage) this.#state.usage = { ...(this.#state.usage || {}), ...responsesUsage(value.usage) }; return; } if (value.type === "message_stop") { if (this.#state.messageStopped) { try { this.#requestContext.observeReasoningProtocol?.({ code: "event_after_terminal" }); } catch {} return; } this.#state.messageStopped = true; return this.#terminal(this.#state.stopReason === "max_tokens" ? "incomplete" : "completed"); } if (value.type === "error") return this.#terminal("failed", value.error?.type === "overloaded_error" ? "provider_overloaded" : "provider_error"); fail("provider_response_malformed"); }
  #messageStart(value) { if (this.#state.messageStarted) fail("provider_response_malformed"); const message = object(value.message); if (typeof message.id !== "string" || !message.id || typeof message.model !== "string" || !message.model) fail("provider_response_malformed"); this.#state.messageStarted = true; this.#state.id = message.id; this.#state.model = message.model; this.#state.created = message.created_at; this.#state.usage = responsesUsage(message.usage); this.#emit("response.created", { type: "response.created", response: { id: this.#state.id, object: "response", status: "in_progress", model: this.#state.model, ...(this.#state.created === undefined ? {} : { created_at: this.#state.created }), output: [], ...(this.#state.usage === undefined ? {} : { usage: this.#state.usage }) } }); }
  #start(value) { const block = object(value.content_block); const providerIndex = value.index; if (!Number.isSafeInteger(providerIndex) || providerIndex < 0 || this.#state.blockProviderIndexes.has(providerIndex)) fail("provider_response_malformed"); if (this.#state.items.size >= MAX_ITEMS) fail("provider_response_malformed"); this.#state.blockProviderIndexes.add(providerIndex); const index = this.#state.nextOutputIndex++; const type = block.type; if (type === "thinking" && !this.#key) fail("thinking_signature_missing"); const id = type === "thinking" ? reasoningItemId(this.#state.id, index) : type === "text" ? generatedTextId(this.#state.id, index) : (typeof block.id === "string" && block.id ? block.id : `call_${index}`); const item = { providerIndex, index, type, id, text: "", signature: "", initialInput: block.input && typeof block.input === "object" ? block.input : undefined, args: "", closed: false, output: undefined }; if (type === "thinking") { item.output = { type: "reasoning", id, status: "in_progress", summary: [], content: [] }; this.#emit("response.output_item.added", { type: "response.output_item.added", output_index: index, item: item.output }); this.#emit("response.reasoning_summary_part.added", { type: "response.reasoning_summary_part.added", item_id: id, output_index: index, summary_index: 0, part: { type: "summary_text", text: "" } }); } else if (type === "text") { item.output = { type: "message", id, status: "in_progress", role: "assistant", content: [] }; this.#emit("response.output_item.added", { type: "response.output_item.added", output_index: index, item: item.output }); this.#emit("response.content_part.added", { type: "response.content_part.added", item_id: id, output_index: index, content_index: 0, part: { type: "output_text", text: "", annotations: [] } }); } else if (type === "tool_use") { if (typeof block.name !== "string" || !block.name) fail("provider_response_malformed"); item.output = { type: "function_call", id, call_id: id, name: block.name, arguments: "" }; this.#emit("response.output_item.added", { type: "response.output_item.added", output_index: index, item: item.output }); } else fail("unsupported_anthropic_block"); this.#state.items.set(providerIndex, item); }
