...
export async function createLocalRuntime(env, { sourceCommit, allowReleased = false, requireSwift = true, installerRunner, providerFixtureFactory = createLocalProviderFixture } = {}) {
  ...
  const runtime = { prepare, stop, start, dispose, callbacks: {
    ...
    browserSession: async () => { const key = installedCallerKey(), port = env.target.ports.router; const response = await fetch(caller.panelSessionUrl(port, key), { method: "POST", headers: { host: `${env.target.host}:${port}`, authorization: `Bearer ${key}`, "content-type": "application/json" }, body: "{}" }); const payload = await response.json().catch(() => ({})); if (!response.ok || typeof payload?.nonce !== "string") throw new Error(`isolated browser session mint failed (${response.status})`); return caller.panelBootstrapUrl(port, payload.nonce); },
