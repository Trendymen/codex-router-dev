...
export function readInstalledCallerSecret(env, { validSecret = (value) => typeof value === "string" && /^[A-Za-z0-9_-]{32,}$/.test(value) } = {}) {
  ...
  try {
    ...
    if ((entry.mode & 0o777) !== 0o600) throw new Error("isolated caller capability must have private mode 0600");
...
export function guardIsolatedRuntimeCallback(isDisposed, callback) {
  return async (...args) => {
    if (isDisposed()) throw new Error("isolated runtime is disposed");
...
export async function createLocalProviderFixture() {
  const attempts = [];
...
export async function createLocalRuntime(env, { sourceCommit, allowReleased = false, requireSwift = true } = {}) {
  ...
  let fixture; let router; let processState; let callerKey; let disposed = false; const pids = [];
  ...
  const start = async () => { if (disposed) throw new Error("isolated runtime is disposed"); if (ownedProcessAlive(processState, router)) return { pid: router.pid, label: env.target.routerLabel, process: processState }; await portsFreeEventually(env.target.ports); fixture ||= await createLocalProviderFixture();
